...
delimiter &&
	...
	begin
		select s.FullName,d.DeptName from Department d join student s  on  d.DeptID=s.DeptID
		where  d.DeptID= 'IT';
    end$$
delimiter ;
...
as
select d.DeptID as'depid' , count(d.DeptID)  as 'total'
from student s 
join  Department d 
on s.DeptID=d.DeptID  
group by d.DeptID ;

select  max(total) from View_StudentCountByDept ;

-- cau 5:
delimiter &&
	create procedure GetTopScoreStudent(p_CourseID)
	begin
